...
const Input = ({ objValue, onChange, index, deleteField }) => {
  ...
  return (
    <div className="input-group">
      ...
      <div className="input">
        {type === "select" ? (
          <select name= "select" id={index} onChange={(e) => onChange(e, index, type)} label = {label} className="select" value={value}>
            {options?.map((rd, idx) => (
          ...
          <>
            <textarea name="text-area" id="" cols="30" rows="10" onChange={(e) => onChange(e, index,type)} value={value || ""}/>
          </>
        ) : (
          (
            <input
